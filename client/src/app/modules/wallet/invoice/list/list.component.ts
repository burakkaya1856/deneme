...
export class InvoiceListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.walletService.getInvoices(requestData).subscribe(data => {
      this.invoiceList = data.items;
      this.isLoaded = true;
      this.pagination = {
        total: data.total,
        size: data.size,
      ...
      this.emptyTransaction = !this.invoiceList.length ? true : false;
    });
  ...
  pageChanged(event: PageChangedEvent): void {
    ...
    this.getInvoices(requestData);
  }

  searchInvoice(event: string): void {
  ...
  getInvoices(invoiceParams): void {
    const loadTimeOut = setTimeout(() => {
      this.isLoaded = false;
    }, 500);

    this.walletService.getInvoices(invoiceParams).subscribe(data => {
      this.invoiceList = data.items;
      this.isLoaded = true;
