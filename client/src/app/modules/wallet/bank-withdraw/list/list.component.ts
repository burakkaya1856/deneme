...
export class ListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.walletService.bankWithdraws(requestData).subscribe(data => {
      this.withdrawList = data.items;
      this.isLoaded = true;
      this.pagination = {
        total: data.total,
        size: data.size,
  ...
  searchWithdraw(event: string): void {
  ...
  getWithdraws(withdrawParams): void {
    const loadTimeOut = setTimeout(() => {
      this.isLoaded = false;
    }, 500);

    this.walletService.bankWithdraws(withdrawParams).subscribe(data => {
      this.withdrawList = data.items;
      this.isLoaded = true;
